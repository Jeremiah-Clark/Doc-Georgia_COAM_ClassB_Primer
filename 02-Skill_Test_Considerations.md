...
- Though it is uncommon in Georgia COAM, some Nudge games can be configured with the nudge arrows present after every spin, even if there is no potential Win.
  - In these cases, pressing the **Play** button without nudging will start the next game (this is the only exception to the rule that the **Play** button is disabled during a Skill Test).
